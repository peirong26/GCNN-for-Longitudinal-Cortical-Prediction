...
from utilities_dataset_036inout import Dataset
...
files_train_in  = open(os.path.join(path_exp_info, 'files_train_036_in.txt'), 'w')
files_test_in   = open(os.path.join(path_exp_info, 'files_test_036_in.txt'), 'w')
files_train_out = open(os.path.join(path_exp_info, 'files_train_036_out.txt'), 'w')
files_test_out  = open(os.path.join(path_exp_info, 'files_test_036_out.txt'), 'w')
...
# dataset loading
ds = Dataset(os.path.join(path_exp_info, 'files_train_036_in.txt'), os.path.join(path_exp_info, 'files_train_036_out.txt'),
             os.path.join(path_exp_info, 'files_test_036_in.txt'), os.path.join(path_exp_info, 'files_test_036_out.txt'),
             path_flags_3,
             path_flags_6,
             path_descs_0,
             path_patches,
...
############ BRIDGE ############
desc_3_in    = LL.DenseLayer(net1_3_2_in, n_out, nonlinearity = None)
################################

net_concat_in = LL.concat([net1_3_2_in, desc_3_in], axis = 1)


# Step 1: patch operator layer
net2_1_1_in  = ApplyDescLayerMask([net, net_concat_in, idx_node_l1_m6, idx_neighbors_l1_m6]) 
# Step 2: convolution layer
...
############ BRIDGE ############
desc_3_out    = LL.DenseLayer(net1_3_2_out, n_out, nonlinearity = None)
################################

net_concat_out = LL.concat([net1_3_2_out, desc_3_out], axis = 1)


# Step 1: patch operator layer
net2_1_1_out  = ApplyDescLayerMask([net, net_concat_out, idx_node_l1_m6, idx_neighbors_l1_m6]) 
# Step 2: convolution layer
...
cost = cost_dataterm_3_in + cost_dataterm_3_out + cost_dataterm_3_thick + cost_dataterm_6_in + cost_dataterm_6_out + cost_dataterm_6_thick + cost_reg
...
funcs = dict()
funcs['train_f'] = theano.function([flag_3, flag_6, desc_0_in.input_var, desc_0_out.input_var, P_rho.input_var, P_theta.input_var, target_3_in, target_3_out, target_6_in, target_6_out, thick_3, thick_6, \
                                ...
                                [cost, cost_dataterm_3_in, cost_dataterm_3_out, cost_dataterm_3_thick, cost_dataterm_6_in, cost_dataterm_6_out, cost_dataterm_6_thick, cost_reg, grad_norm], 
                                updates = updates_f, 
                                allow_input_downcast='True',
                                on_unused_input = 'warn')

funcs['train_s'] = theano.function([flag_3, flag_6, desc_0_in.input_var, desc_0_out.input_var, P_rho.input_var, P_theta.input_var, target_3_in, target_3_out, target_6_in, target_6_out, thick_3, thick_6, \
                                 idx_node_l1_m3.input_var, idx_neighbors_l1_m3.input_var, idx_node_l2_m3.input_var, idx_neighbors_l2_m3.input_var, \
                                 idx_node_l1_m6.input_var, idx_neighbors_l1_m6.input_var, idx_node_l2_m6.input_var, idx_neighbors_l2_m6.input_var, idx_node_l3_m6.input_var, idx_neighbors_l3_m6.input_var],
                                [cost, cost_dataterm_3_in, cost_dataterm_3_out, cost_dataterm_3_thick, cost_dataterm_6_in, cost_dataterm_6_out, cost_dataterm_6_thick, cost_reg, grad_norm], 
                                updates = updates_s, 
...
funcs['pred'] = theano.function([flag_3, flag_6, desc_0_in.input_var, desc_0_out.input_var, P_rho.input_var, P_theta.input_var, target_3_in, target_3_out, target_6_in, target_6_out, thick_3, thick_6, \
                                 idx_node_l1_m3.input_var, idx_neighbors_l1_m3.input_var, idx_node_l2_m3.input_var, idx_neighbors_l2_m3.input_var, \
                                 idx_node_l1_m6.input_var, idx_neighbors_l1_m6.input_var, idx_node_l2_m6.input_var, idx_neighbors_l2_m6.input_var, idx_node_l3_m6.input_var, idx_neighbors_l3_m6.input_var],
                                [cost, cost_dataterm_3_in, cost_dataterm_3_out, cost_dataterm_3_thick, cost_dataterm_6_in, cost_dataterm_6_out, cost_dataterm_6_thick, cost_reg, grad_norm, pred_3_in, pred_3_out, pred_6_in, pred_6_out], 
...
def test_model(x_, n_vert):
    ...
    while(1):
        ...
        desc_in_sel      = x_[2][list_all_neighbors, :]
        desc_out_sel     = x_[3][list_all_neighbors, :]
        target_3_in_sel  = x_[8][list_selected_nodes]
        target_3_out_sel = x_[9][list_selected_nodes]
        thick_3_sel      = x_[12][list_selected_nodes]

        target_6_in_sel  = x_[10][list_selected_nodes]
        target_6_out_sel = x_[11][list_selected_nodes]
        thick_6_sel      = x_[13][list_selected_nodes]
        
        if (idx_compute_label_mask_l2_m3.shape!=()):
            target_3_in_sel  = np.asarray(target_3_in_sel)[idx_compute_label_mask_l2_m3]
            target_3_out_sel = np.asarray(target_3_out_sel)[idx_compute_label_mask_l2_m3]
            thick_3_sel      = np.asarray(thick_3_sel)[idx_compute_label_mask_l2_m3]
        else:
            target_3_in_sel  = [np.asarray(target_3_in_sel)[np.asscalar(idx_compute_label_mask_l2_m3)]]
            target_3_out_sel = [np.asarray(target_3_out_sel)[np.asscalar(idx_compute_label_mask_l2_m3)]]
            thick_3_sel      = [np.asarray(thick_3_sel)[np.asscalar(idx_compute_label_mask_l2_m3)]]

            idx_compute_label_mask_l2_m3 = [np.asscalar(idx_compute_label_mask_l2_m3)]
        ...
        all_pred_3_in[list_selected_nodes[idx_compute_label_mask_l2_m3],:]  = tmp[9]
        all_pred_3_out[list_selected_nodes[idx_compute_label_mask_l2_m3],:] = tmp[10]  
        all_pred_6_in[list_selected_nodes[idx_compute_label_mask_l3_m6],:]  = tmp[11] 
        all_pred_6_out[list_selected_nodes[idx_compute_label_mask_l3_m6],:] = tmp[12] 

        overall_cost += tmp[0]*len(idx_compute_label_mask_l2_m6)

        cost_3_in    += tmp[1]*len(idx_compute_label_mask_l2_m3)
        cost_3_out   += tmp[2]*len(idx_compute_label_mask_l2_m3)
        cost_3_thick += tmp[3]*len(idx_compute_label_mask_l2_m3)
        cost_6_in    += tmp[4]*len(idx_compute_label_mask_l3_m6)
        cost_6_out   += tmp[5]*len(idx_compute_label_mask_l3_m6)
        cost_6_thick += tmp[6]*len(idx_compute_label_mask_l3_m6)
    ...
    return [overall_cost*1.0/n_vert, cost_3_in*1.0/n_vert, cost_3_out*1.0/n_vert, cost_3_thick*1.0/n_vert, cost_6_in*1.0/n_vert, cost_6_out*1.0/n_vert, cost_6_thick*1.0/n_vert, all_pred_3_in, all_pred_3_out, all_pred_6_in, all_pred_6_out]
...
reg_exp_dump_model = os.path.join(path_dumps, 'pkl/06/out_in/gcnn036_%d_epoch=%d.pkl')

path_output_3 = os.path.join(path_dumps, 'mat/03/out_in/gcnn036_batch_dim=%d') % (batch_dim)
path_output_6 = os.path.join(path_dumps, 'mat/06/out_in/gcnn036_batch_dim=%d') % (batch_dim)
...
for i in range(max_num_iter):
    ...
    for x_ in ds.train_iter():    
        ...
        cost_train_3_in.append(tmp[1]*len(idx_compute_label_mask_l2_m3))
        cost_train_3_out.append(tmp[2]*len(idx_compute_label_mask_l2_m3))
        cost_train_3_thick.append(tmp[3]*len(idx_compute_label_mask_l2_m3))
        cost_train_6_in.append(tmp[4]*len(idx_compute_label_mask_l3_m6))
        cost_train_6_out.append(tmp[5]*len(idx_compute_label_mask_l3_m6))
        cost_train_6_thick.append(tmp[6]*len(idx_compute_label_mask_l3_m6))

        grad_norm_train.append(tmp[8]*len(idx_compute_label_mask_l3_m6))
    ...
    if (len(cost_train_avg) % test_interval) == 0: #printing the test statistics
        for k,d in enumerate(ds.test_fwd()):
            ...
            tic = time.time()
            cost_test, cost_test_3_in, cost_test_3_out, cost_test_3_thick, cost_test_6_in, cost_test_6_out, cost_test_6_thick, tmp_pred_3_in, tmp_pred_3_out, tmp_pred_6_in, tmp_pred_6_out = test_model(test_shape, n_vert)  
...
funcs['train'] = theano.function([flag_3, flag_6, desc_0_in.input_var, desc_0_out.input_var, P_rho.input_var, P_theta.input_var, target_3_in, target_3_out, target_6_in, target_6_out, thick_3, thick_6, \
                                 idx_node_l1_m3.input_var, idx_neighbors_l1_m3.input_var, idx_node_l2_m3.input_var, idx_neighbors_l2_m3.input_var, \
                                 idx_node_l1_m6.input_var, idx_neighbors_l1_m6.input_var, idx_node_l2_m6.input_var, idx_neighbors_l2_m6.input_var, idx_node_l3_m6.input_var, idx_neighbors_l3_m6.input_var],
                                [cost, cost_dataterm_3_in, cost_dataterm_3_out, cost_dataterm_3_thick, cost_dataterm_6_in, cost_dataterm_6_out, cost_dataterm_6_thick, cost_reg, grad_norm], 
                                updates = updates, 
...
for i,d in enumerate(ds.test_fwd()):
    ...
    print(fname_6_in),
    print(fname_6_out),
    current_cost, current_cost_3_in, current_cost_3_out, current_cost_3_thick, current_cost_6_in, current_cost_6_out, current_cost_6_thick, current_pred_3_in, current_pred_3_out, current_pred_6_in, current_pred_6_out = test_model_pred(d, n_vert) 
