...
load('/Users/peirong/Documents/Cycle/036/Results/affine/li_3.mat');
pred_li = pred_3;
load('/Users/peirong/Documents/Cycle/036/Results/affine/lo_3.mat');
pred_lo = pred_3; 
load('/Users/peirong/Documents/Cycle/036/Results/affine/ri_3.mat');
pred_ri = pred_3;
load('/Users/peirong/Documents/Cycle/036/Results/affine/ro_3.mat');
pred_ro = pred_3; 
...
Lh_Inner.Thickness_Error = sqrt(tmp_l) - 0.5*mean(tmp_l);
Lh_Outer.Thickness_Error = sqrt(tmp_l) - 0.5*mean(tmp_l);
Rh_Inner.Thickness_Error = sqrt(tmp_r) - 0.5*mean(tmp_r);
Rh_Outer.Thickness_Error = sqrt(tmp_r) - 0.5*mean(tmp_r);
...
tmp_li = pred_li - [gt_li.X,gt_li.Y,gt_li.Z];
tmp_lo = pred_lo - [gt_lo.X,gt_lo.Y,gt_lo.Z];
tmp_ri = pred_ri - [gt_ri.X,gt_ri.Y,gt_ri.Z];
tmp_ro = pred_ro - [gt_ro.X,gt_ro.Y,gt_ro.Z];
...
err_li = abs(err_li - 0.3*mean(abs(err_li)));
err_lo = abs(err_lo - 0.3*mean(abs(err_lo)));
err_ri = abs(err_ri - 0.3*mean(abs(err_ri)));
err_ro = abs(err_ro - 0.3*mean(abs(err_ro)));
...
% Write vtk
path_write_vtk_li = fullfile(path_write_vtk, ['Lh_Inner_AF3','.vtk']);
path_write_vtk_lo = fullfile(path_write_vtk, ['Lh_Outer_AF3','.vtk']);
path_write_vtk_ri = fullfile(path_write_vtk, ['Rh_Inner_AF3','.vtk']);
path_write_vtk_ro = fullfile(path_write_vtk, ['Rh_Outer_AF3','.vtk']);
