...
pFieldNameSet = fieldnames(s);
for i=1:numel(pFieldNameSet)
    strName = pFieldNameSet{i};
    if(strcmpi(strName,'vertices') || strcmpi(strName,'faces'))
        continue;
    end
    
    p_attrib = getfield(s,strName);
    if(isempty(p_attrib))
        continue;
    end
    ...
    dataName =strName;
    if(size(p_attrib,2)==1)
        dataType = class(p_attrib);
        if(isempty(strfind(dataType,'int')))
            fprintf(fid,'SCALARS %s %s %d\n',dataName,'float',size(p_attrib,2));
            fprintf(fid,'LOOKUP_TABLE %s\n',lut_name);
            fmt = repmat('%f ',1,size(p_attrib,2)); 
            fmt(end) = ''; 
            write_data(fid,[fmt '\n'],'float32',p_attrib);
        else
            fprintf(fid,'SCALARS %s %s %d\n',dataName,'int',size(p_attrib,2));
            fprintf(fid,'LOOKUP_TABLE %s\n',lut_name);
            fmt = repmat('%d ',1,size(p_attrib,2)); 
            fmt(end) = ''; 
            write_data(fid,[fmt '\n'],'int32',p_attrib);
        end
        ...
        p_attrib = uint8(p_attrib)';
 %       fmt = repmat('%d ',1,size(p_attrib,2)); 
        
        fmt = repmat('%d %d %d \n',1,size(p_attrib,2)); 
 %       fmt(end) = ''; 
        write_data(fid,fmt,'uchar',p_attrib);
%         fprintf(fid,'%d %d %d\n','uchar',p_attrib);
%         for nRow = 1:size(p_attrib,1)
%             write_data(fid,'%d','uchar',p_attrib(nRow,1));
%             fprintf(fid,' ');
%             write_data(fid,'%d','uchar',p_attrib(nRow,2));
%             fprintf(fid,' ');
%             write_data(fid,'%d','uchar',p_attrib(nRow,3));
%             fprintf(fid,'\n');
%         end
