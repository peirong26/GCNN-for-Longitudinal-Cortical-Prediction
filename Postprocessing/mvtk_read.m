        ...
        while true
            ...
            switch D
                ...
                case {'POINT_DATA','CELL_DATA'}
                    ...
                    [N,l] = strtok(l(2:end)); N = str2double(N);
                    while true
                        l = fgetl(fid); % SCALARS dataName dataType numComp
                        if ~ischar(l), break; end
                        if isempty(l), continue; end
                        [P,l] = strtok(l);
                        if(strcmpi(P,'scalars'))
                            [dataName,l] = strtok(l(2:end));
                            [dataType,l] = strtok(l(2:end));
                            ...
                            fmt = [fmt(1:end-1) '\n'];
                            attrib_data = textscan(fid,fmt,N,'CollectOutput',true);
                            M.(dataName) = attrib_data{1};
                        elseif(strcmpi(P,'vectors'))
                            [dataName,dataType] = strtok(l(2:end));
                            fmt = [];
                            dataType=lower(dataType);
                            if(~isempty(strfind(dataType,'float')))
                                fmt = repmat('%f ',1,3);
                            elseif(~isempty(strfind(dataType,'unsigned_char')))
                                fmt = repmat('%d ',1,3);
                            end
                            fmt = [fmt(1:end-1) '\n'];
                            attrib_data = textscan(fid,fmt,N,'CollectOutput',true);
